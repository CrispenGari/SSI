...
# Model paths
PYTORCH_SSI_MODEL_PATH = os.path.join(
    os.getcwd(), f"api/models/pytorch/static/{MODEL_NAME}"
)


class Specie:
    def __init__(
        self, id: int, class_: str, specie_name: str, common_name: str
    ) -> None:
        self.id = id
        self.class_ = class_
...
class Prediction:
    def __init__(
        self, label: int, probability: float, class_name: str, specie: Specie
    ) -> None:
        self.label = label
